/**
 * script/screenshot-sprint-10.ts
 *
 * Renders the Sprint 10 GoalSolverProSection to static HTML using the same
 * fixture as the regression test, wraps it in the production CSS bundle, and
 * uses playwright-core (already-installed Chromium) to capture full-page
 * screenshots.
 *
 * Outputs:
 *   screenshots/sprint10-rich.png     (populated state)
 *   screenshots/sprint10-empty.png    (no targets supplied)
 *
...
<title>Sprint 10 — Goal Solver Pro</title>
<style>${css}</style>
<style>
...
(async () => {
  ...
  const sprint7 = buildTruePortfolioOptimizer({ canonicalLedger: FIXTURE, goalSolverInputs: GOAL_INPUTS });
  ...
  const richTargets = {
    targetFireYear: 2045,
    targetNetWorth: 3_500_000,
    targetPassiveIncomeAnnual: 96_000,
    targetPropertyCount: 2,
    targetDebtCeiling: 2_000_000,
    targetMonthlyContributionLimit: 8_000,
    targetLiquidityMinimum: 3,
    targetRetirementYear: 2050,
  };

  const richResult = buildGoalSolverPro({
    canonicalLedger: FIXTURE,
    canonicalFire,
    sprint7Result: sprint7,
  ...
  const emptyResult = buildGoalSolverPro({
    canonicalLedger: FIXTURE,
    canonicalFire,
    sprint7Result: sprint7,
    sprint8Result: sprint8,
  ...
  fs.writeFileSync(path.join(SHOT_DIR, "sprint10-rich.html"), richDoc);
  fs.writeFileSync(path.join(SHOT_DIR, "sprint10-empty.html"), emptyDoc);
  ...
  for (const [name, doc] of [
    ["sprint10-rich", richDoc],
    ["sprint10-empty", emptyDoc],
  ] as const) {
