/**
 * forecastFreshness.ts — FWL Remediation Phase A-4 (shared boundary).
 *
 * Moved out of client/src/lib into shared/ so the server can import it without
 * crossing the client boundary (PR #88 review item #4). The client re-exports
 * from here, so existing `@/lib/forecastFreshness` imports keep working.
 *
